...
async function fetchLeague(league) {
  const response = await fetch(
    `https://api.the-odds-api.com//v4/sports/${league}/scores/?apiKey=${process.env.REACT_APP_ODDS_API_KEY}&regions=us&markets=h2h,spreads,totals&bookmakers=fanduel&dateFormat=unix&daysFrom=3`,
  );
...
export default async function handler(req, res) {
  ...
  switch (method) {
    ...
    case "PUT": {
      const refTime = ref(db, `scores/updated`);
